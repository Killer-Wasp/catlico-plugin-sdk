...
If `MyPlugin` tried `ctx.api.get_case(1)` without `read:case` in the manifest, that call
...
yours. `tests/test_testing.py` in this repo has the complete working version against a real
`EnrichingPlugin`.
