...
## `catlico-plugin new PLUGIN_ID [--dir DIR] [--name NAME]`

Scaffolds a fresh, valid plugin directory tree at `DIR/PLUGIN_ID/` (default `DIR` is the
current directory): `catlico-plugin.toml`, `pyproject.toml`, a root `main.py` (the `Catlico`
app + decorators), `src/<pkg>/{__init__.py,plugin.py}` (your logic as plain functions), a
starter `tests/test_plugin.py`, and a `.github/workflows/ci.yml` that runs `uv sync --frozen`,
`pytest`, and `catlico-plugin validate` on every push and pull request. Refuses to run if the
...
matching `abuseipdb` → `abuseipdb_plugin`. `--name` sets the manifest's display `name`
(default: the plugin id).

The generated manifest passes `validate_manifest` with no errors and `manifest_warnings` with
no warnings out of the box; the generated `main.py` wires an `@catlico.event("observable.created")`
handler to a `process()` function in `plugin.py`, with clear `TODO`s where you plug in real logic.

> **Where to run it.** The generated `pyproject.toml` pins the SDK at
> `../../catlico-plugin-sdk` (a local-`uv` dev convenience), so scaffold from inside
> `catlico-plugins/` (or another directory two levels below the workspace root, next to
> `catlico-plugin-sdk`) for that path dependency to resolve. A standalone plugin repo replaces
> that source with a git pin — see the comment in the generated `pyproject.toml`.
...
Imports the plugin's `Catlico` app via its manifest entrypoint, checks that the manifest's
`triggers` match the app's registered `@catlico.event` handlers, parses `EVENT.json` into a
`PluginEvent`, and dispatches it through the app against a `FakeContext` — then prints emitted
results, progress, uploaded files, and any permission violations.
...
| Exit code | When |
|---|---|
| `0` | run ended `success`, or `skipped` (a matcher rejected the event, `SkipRun`, or no handler matched) |
| `1` | hard error before the plugin ran (missing/invalid manifest, triggers≠handlers, unreadable event, bad fixture, import failure), or the run ended `failure` |
